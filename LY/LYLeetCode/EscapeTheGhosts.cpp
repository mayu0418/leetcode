...
bool escapeGhosts(int** ghosts, int ghostsRowSize, int *ghostsColSizes, int* target, int targetSize) {
    bool canEscape=true;
    int sumStep=abs(target[0])+abs(target[1]);
    for (int i=0; i<ghostsRowSize; i++) {
        if (abs(ghosts[i][0]-target[0])+abs(ghosts[i][1]-target[1])<=sumStep) {
            canEscape=false;
        }
    }
    return canEscape;
...
int main(int argc, const char * argv[]) {
    int **ghosts=(int**)malloc(sizeof(int*)*2);
    ghosts[0]=(int*)malloc(sizeof(int)*2);
    ghosts[1]=(int*)malloc(sizeof(int)*2);
    ghosts[0][0]=1;
    ghosts[0][1]=0;
    ghosts[1][0]=0;
    ghosts[1][1]=3;
    ...
    int ghostsRowSize=2;
    int *ghostsColSizes=(int*)malloc(sizeof(int)*2);
    for (int i=0; i<ghostsRowSize; i++) {
        ghostsColSizes[i]=2;
    }
    ...
    int targetSize=1;
    escapeGhosts(ghosts, ghostsRowSize, ghostsColSizes, target, 1);
}
